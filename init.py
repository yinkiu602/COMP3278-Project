...
def send_email_fun(input_text, to_email):
    # TODO fix email and application key
    email="@gmail.com"
    application_key = ""
    email_obj = MIMEMultipart()
    email_obj["From"] = email
    email_obj["To"] = to_email
    email_obj["subject"] = "Course Material"
    message_content = "(This is an automatically generated email. Do not reply.)<br><br>Here is the course material you requested:<br><br>" + input_text + "<br><br>Best regards,<br>HKU Login System"
    email_obj.attach(MIMEText(message_content, "html"))
    with smtplib.SMTP(host="smtp.gmail.com", port="587") as smtp: 
        smtp.ehlo()  
        smtp.starttls()  
        smtp.login(email, application_key)  
...
def fetch_event(student_id):
    ...
    #result = fetch_sql(f"SELECT C.course_id, C.classroom_address, C.start_time, C.end_time FROM Enroll E, Course_schedule C WHERE E.student_id = {student_id} AND E.course_id = C.course_id AND C.weekday = 3")
...
def whoami():
    if "face_id" not in request.cookies:
        return redirect("/")
    name = fetch_sql(f"SELECT first_name FROM Student WHERE face_id = '{request.cookies['face_id']}'")[0][0]
    return name
...
def send_email():
    html_text = request.args.get("html_text")
    student_id = request.cookies["student_id"]
    email = fetch_sql(f"SELECT email FROM Student WHERE student_id = '{student_id}'")[0][0]
    send_email_fun(html_text, email)
    return ""
